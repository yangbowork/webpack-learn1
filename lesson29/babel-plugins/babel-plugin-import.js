...
const visitor = {
  ...
  ImportDeclaration: {
    // 当进入这个节点的时候，执行此函数 节点路径path，state代表节点的状态
    enter(path, state = { opts: {} }) {
      let { opts } = state;
      const { node } = path; // ImportDeclaration
      const specifiers = node.specifiers;
      const source = node.source; // loadsh
      // 只处理lodash模块，别的不管，并且不是默认导入的话才需要处理，否则绕过去
      if (
        ...
        let defaultImportDeclaration = specifiers.map((specifier) => {
          let importDefaultSpecifier = t.importDefaultSpecifier(
            specifier.local
          );
